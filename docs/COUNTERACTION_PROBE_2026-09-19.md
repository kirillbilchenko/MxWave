...
The retained research branch provides `mxwave-counteraction-probe`; it is not
installed by the production package. The two commands differ only in
`--sequence-offset` and output path. Exact Spark paths, image digest, commit,
