...
_SKIPPED_ASSETS = frozenset({"config.json", "model.safetensors.index.json"})
...
def build_model_card(manifest: dict[str, Any]) -> str:
    ...
    hessian_rounding_sweeps = manifest.get("hessian_rounding_sweeps", 0)
    hessian_error_feedback = manifest.get("hessian_error_feedback", False)
    hessian_feedback_damp_percent = manifest.get("hessian_feedback_damp_percent")
    hessian_feedback_activation_order = manifest.get("hessian_feedback_activation_order")
    hessian_feedback_max_mse_ratio = manifest.get("hessian_feedback_max_mse_ratio")
    feedback_selection = manifest.get("feedback_selection_calibration")
    feedback_selection = feedback_selection if isinstance(feedback_selection, dict) else {}
    ...
    feedback_selection_sqnr_data = manifest.get("feedback_selection_weighted_sqnr_db")
    feedback_selection_sqnr_data = (
        feedback_selection_sqnr_data
        if isinstance(feedback_selection_sqnr_data, dict)
        else {}
    )
    feedback_selection_sqnr_count = feedback_selection_sqnr_data.get("count", 0)
    feedback_selection_sqnr_minimum = feedback_selection_sqnr_data.get("minimum")
    feedback_selection_sqnr_mean = feedback_selection_sqnr_data.get("mean")
    feedback_selection_sqnr_result = (
        f"{float(feedback_selection_sqnr_minimum):.2f} dB minimum / "
        f"{float(feedback_selection_sqnr_mean):.2f} dB mean across "
        f"{feedback_selection_sqnr_count} bounded row samples"
        if isinstance(feedback_selection_sqnr_minimum, int | float)
        and isinstance(feedback_selection_sqnr_mean, int | float)
        else "not applicable"
    )
    ...
    feedback_selection_text = (
        f"independent block-Hessian selection on "
        f"{feedback_selection.get('num_sequences', 'unknown')} sequences at offset "
        f"{feedback_selection.get('sequence_offset', 'unknown')}"
    ...
    body = [
        f"# {title_source} — mxstream {method.upper()} MXFP4",
        "",
        (
            "This checkpoint was generated by "
            f"mxstream `{producer.get('version', 'unknown')}` from {source_text}."
        ),
        ...
        f"| Hessian rounding sweeps | {hessian_rounding_sweeps} |",
        f"| Hessian error feedback | {hessian_error_feedback} |",
        f"| Hessian feedback damping | {hessian_feedback_damp_percent} |",
        f"| Static activation order | {hessian_feedback_activation_order} |",
        f"| Feedback MSE trust-region ratio | {hessian_feedback_max_mse_ratio} |",
        f"| Feedback candidate selection | {feedback_selection_text} |",
        ...
        f"| Feedback-selection SQNR | {feedback_selection_sqnr_result} |",
        "",
        (
            "The exact targeted and ignored module lists, per-tensor SQNR samples, source "
            "identity, and byte counts are recorded in `mxstream-manifest.json`."
        ),
        "",
        "## Usage",
        "",
        ...
        (
            "- Hessian code rounding is experimental; one full-strength sweep regressed the "
            "initial downstream pilot despite improving its local reconstruction objective."
            if isinstance(hessian_rounding_sweeps, int) and hessian_rounding_sweeps > 0
            else "- Hessian code rounding is disabled by default pending stronger validation."
        ),
        (
            "- Block-local Hessian error feedback is experimental and is not equivalent to "
            "full-layer, sequential GPTQ."
            if hessian_error_feedback is True
            else "- Block-local Hessian error feedback is disabled by default pending validation."
        ),
        (
            "- Feedback-selection calibration chooses weight codes; it is not an independent "
            "model-quality evaluation set."
            if feedback_selection
            else "- Independent feedback-selection calibration was not used."
        ),
        (
...
def record_runtime_validation(
    ...
    envelope and atomically keeps ``mxstream-manifest.json`` and ``README.md`` in sync.
    """
    output = Path(output_dir)
    manifest_path = output / "mxstream-manifest.json"
    if not manifest_path.is_file():
        raise FileNotFoundError(f"mxstream manifest is missing: {manifest_path}")
    raw_manifest = json.loads(manifest_path.read_text())
    if not isinstance(raw_manifest, dict):
        raise TypeError("mxstream manifest must contain a JSON object")
